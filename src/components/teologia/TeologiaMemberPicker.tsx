/**
 * Seletor de pessoa sobre public.members, via RPC mínima e escopada
 * (search_theology_members). A busca acontece no servidor e devolve apenas
 * nome/código — nunca CPF, telefone ou milhares de perfis para o navegador.
 * Mesmo padrão de DiscipuladoMemberPicker.tsx (OPERAÇÃO 2) — reaproveitado
 * como PADRÃO, não como tabela/componente compartilhado diretamente, pois a
 * busca de Teologia usa sua própria RPC escopada por theology.read.
 */
import { useEffect, useState } from "react";
import { Search, Loader2, User } from "lucide-react";
import { searchTheologyMembers, type TheologyMemberLabel } from "@/lib/theology/service";

export function TeologiaMemberPicker({ organizationId, onSelect, excludeIds = [] }: {
  organizationId: string;
  onSelect: (member: TheologyMemberLabel) => void;
  excludeIds?: string[];
}) {
  const [query, setQuery] = useState("");
  const [members, setMembers] = useState<TheologyMemberLabel[]>([]);
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(async () => {
      setLoading(true);
      setError(null);
      const result = await searchTheologyMembers(organizationId, query);
      if (!cancelled) {
