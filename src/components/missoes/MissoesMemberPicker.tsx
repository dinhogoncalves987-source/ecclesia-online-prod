 ...
 * (search_missions_members). A busca acontece no servidor e devolve apenas
 * nome/código — nunca CPF, telefone ou milhares de perfis para o navegador.
 * Mesmo padrão de TeologiaMemberPicker.tsx (OPERAÇÃO 3), reaproveitado como
 * PADRÃO — Missões usa sua própria RPC escopada por missions.read.
 */
import { useEffect, useState } from "react";
import { Search, Loader2, User } from "lucide-react";
import { searchMissionsMembers, type MissionsMemberLabel } from "@/lib/missions/service";

export function MissoesMemberPicker({ organizationId, onSelect, excludeIds = [] }: {
  organizationId: string;
  onSelect: (member: MissionsMemberLabel) => void;
  excludeIds?: string[];
}) {
  const [query, setQuery] = useState("");
  const [members, setMembers] = useState<MissionsMemberLabel[]>([]);
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(async () => {
      setLoading(true);
      setError(null);
      const result = await searchMissionsMembers(organizationId, query);
      if (!cancelled) {
