...
describe("Operação 1 — member_history (fundação compartilhada)", () => {
  ...
  it("é append-only para authenticated: sem policy ou GRANT direto de INSERT/UPDATE", () => {
    const code = stripSqlComments(sql);
    expect(code).not.toMatch(/CREATE POLICY "member_history capability (insert|update)"/i);
    expect(code).not.toMatch(/GRANT\s+[^;]*(INSERT|UPDATE)[^;]*ON public\.member_history TO authenticated/i);
    expect(code).toMatch(/REVOKE INSERT, UPDATE, DELETE ON public\.member_history FROM authenticated/i);
  });
  ...
  it("não expõe a RPC genérica de auditoria ao navegador; somente service_role/triggers escrevem", () => {
    expect(sql).toMatch(/GRANT EXECUTE ON FUNCTION public\.register_member_history_event\([\s\S]*?\) TO service_role;/);
    expect(sql).not.toMatch(/register_member_history_event\([\s\S]*?\) TO authenticated;/);
    expect(sql).toMatch(
      /REVOKE ALL ON FUNCTION public\.register_member_history_event\([\s\S]*?\) FROM PUBLIC, anon, authenticated;/,
    );
  });

  it("cria idempotência legada por evento, sem colidir fatos diferentes do mesmo registro", () => {
    ...
    expect(idx![0]).toContain("legacy_source");
    expect(idx![0]).toContain("COALESCE(legacy_module");
    expect(idx![0]).toContain("history_type");
    expect(idx![0]).toContain("COALESCE(source_id");
  });
  ...
  it("adiciona a capability members.confidential apenas a church_admin/responsible_pastor (governança), de forma idempotente", () => {
    ...
    expect(update![0]).toMatch(/NOT \('members\.confidential' = ANY\(COALESCE\(permission_keys/);
  });
});
...
describe("Operação 1 — member_occurrences (ocorrências pastorais/administrativas)", () => {
  ...
  it("escreve somente por RPC controlada e não aceita INSERT/UPDATE direto do cliente", () => {
    const code = stripSqlComments(sql);
    expect(code).toMatch(/CREATE OR REPLACE FUNCTION public\.create_member_occurrence/);
    expect(code).toMatch(/CREATE OR REPLACE FUNCTION public\.update_member_occurrence_status/);
    expect(code).toMatch(/REVOKE INSERT, UPDATE, DELETE ON public\.member_occurrences FROM authenticated/);
    expect(code).not.toMatch(/CREATE POLICY "member_occurrences capability (insert|update)"/);
  });
...
describe("Operação 1 — member_ordinations (ordenações e nomeações temporais)", () => {
  ...
  it("guarda contra duplicidade evidente (mesmo membro, mesma função, mesma data de início)", () => {
    ...
    expect(idx![0]).toContain("member_id, lower(btrim(role_or_function)), start_date");
  });
  ...
  it("criação e encerramento passam por RPCs com transição controlada", () => {
    expect(sql).toMatch(/CREATE OR REPLACE FUNCTION public\.create_member_ordination/);
    expect(sql).toMatch(/CREATE OR REPLACE FUNCTION public\.update_member_ordination_status/);
    expect(sql).toMatch(/v_row\.status = 'ativo' AND p_status IN \('encerrado', 'revogado'\)/);
    expect(sql).toMatch(/REVOKE INSERT, UPDATE, DELETE ON public\.member_ordinations FROM authenticated/);
  });
});
...
describe("Operação 1 — member_transfers (transferências)", () => {
  ...
  it("monta a ponta local no banco e restringe as transições de status", () => {
    expect(sql).toMatch(/CREATE OR REPLACE FUNCTION public\.create_member_transfer/);
    expect(sql).toMatch(/CREATE OR REPLACE FUNCTION public\.update_member_transfer_status/);
    expect(sql).toMatch(/v_row\.status = 'solicitada' AND p_status IN \('aprovada', 'rejeitada', 'cancelada'\)/);
    expect(sql).toMatch(/v_row\.status = 'aprovada' AND p_status IN \('concluida', 'cancelada'\)/);
    expect(sql).toMatch(/REVOKE INSERT, UPDATE, DELETE ON public\.member_transfers FROM authenticated/);
  });
});
...
describe("Operação 1 — member_organization_history (vínculo organizacional derivado)", () => {
  ...
  it("faz backfill do vínculo atual para membros que já existiam", () => {
    expect(sql).toMatch(/INSERT INTO public\.member_organization_history/);
    expect(sql).toMatch(/Backfill da fundação institucional/);
