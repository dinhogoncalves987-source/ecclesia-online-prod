...
describe("OPERAÇÃO ESPECIAL — migrations novas (TV/Canal/Chat/OTP)", () => {
  ...
  // ATUALIZAÇÃO 2026-07-30 (restauração de TV Digital/Canal Eclésia — ver
  // src/config/modules.ts): a categoria "staging_feature" foi descontinuada
  // (deve ficar sempre vazia, ver src/config/releaseEnvironmentParity.test.ts)
  // e as 5 migrations desta operação foram promovidas para
  // "production_management" junto com o restante do backend de TV/Canal, que
  // já estava homologado. O teste original ("never promoted by this
  // operation") descrevia um estado anterior à promoção.
  it("classifies the new OTP/TV/Canal modules as production_management (promoted alongside the TV/Canal restoration)", () => {
    const manifest = JSON.parse(read("supabase/migration-manifest.json")) as {
      production_management: string[];
    ...
    for (const name of [
      ...
      expect(manifest.production_management).toContain(name);
    }
  ...
  it("never touches any category already present for pre-existing migrations", () => {
    ...
    expect(manifest.production_management).toContain("20260731140000_missions_history_and_reports.sql");
...
describe("Login por telefone/WhatsApp (Parte D) — contrato de segurança", () => {
  ...
  // A tela administrativa de teste manual de OTP (`LoginOtpTeste`) pertence a
  // uma frente de trabalho separada (login por telefone/WhatsApp) e nunca foi
  // trazida de volta nesta restauração de TV Digital/Canal Eclésia — não
  // existe em src/pages/admin/ nesta branch. As migrations que ela consome
  // (OTP_FOUNDATION/OTP_ADMIN_TEST) já estão validadas nos testes acima.
});
