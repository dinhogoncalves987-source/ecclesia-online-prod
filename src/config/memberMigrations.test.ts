...
describe("Parte 1 — member_family (20260727020000)", () => {
  ...
  it("permite múltiplos filhos/dependentes (índice único considera o nome, não só a relação)", () => {
    ...
    expect(idx![0]).toContain("member_id, relation, lower(btrim(full_name))");
  });

  it("impede reatribuir endereço ou familiar a outro membro/organização por UPDATE", () => {
    expect(sql).toMatch(/CREATE TRIGGER member_family_protect_identity/);
    expect(sql).toMatch(/NEW\.member_id IS DISTINCT FROM OLD\.member_id/);
...
describe("Parte 1 — validação de CPF (20260727030000)", () => {
  ...
  it("cria uma CHECK constraint com validação completa dos dígitos do CPF", () => {
    expect(sql).toMatch(/ADD CONSTRAINT members_cpf_format_check/);
    expect(sql).toMatch(/CREATE OR REPLACE FUNCTION public\.is_valid_cpf/);
    expect(sql).toMatch(/public\.is_valid_cpf\(cpf\)/);
  });
  ...
  it("cria um índice único de CPF por organização, excluindo CPFs pendentes", () => {
    const idx = sql.match(/CREATE UNIQUE INDEX members_org_cpf_unique_idx[\s\S]*?;/);
    expect(idx, "índice members_org_cpf_unique_idx não encontrado").toBeTruthy();
    expect(idx![0]).toContain("organization_id, regexp_replace(cpf");
