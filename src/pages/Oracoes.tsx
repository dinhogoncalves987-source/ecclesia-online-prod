    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-serif font-bold text-foreground">Pedidos de Oração</h1>
            <p className="text-sm text-muted-foreground mt-1">Compartilhe e interceda pelos pedidos da comunidade</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground">Nenhum pedido de oração encontrado</p>
          </div>
        ) : (
            ...
            {filtered.map((req, i) => (
              <motion.div key={req.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1"><User size={12} />{req.is_anonymous ? "Anônimo" : "Membro"}</span>
                  <span className="flex items-center gap-1"><Clock size={12} />{format(new Date(req.created_at), "dd MMM", { locale: ptBR })}</span>
                  <span className="flex items-center gap-1"><Heart size={12} />{req.praying_count} orando</span>
                </div>
        ...
        {showForm && (
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed inset-x-4 top-1/2 -translate-y-1/2 sm:inset-auto sm:left-1/2 sm:-translate-x-1/2 sm:w-full sm:max-w-md bg-card rounded-2xl p-6 shadow-xl z-50">
              <h2 className="text-lg font-serif font-bold mb-4">Novo Pedido de Oração</h2>
              <div className="space-y-3">
                <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Título do pedido" className="w-full px-3 py-2 rounded-lg bg-secondary border border-border text-sm" />
                <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Descrição (opcional)" rows={3} className="w-full px-3 py-2 rounded-lg bg-secondary border border-border text-sm resize-none" />
              ...
              <div className="flex gap-2 mt-4">
                ...
                <button onClick={handleAdd} className="flex-1 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium">Enviar Pedido</button>
              </div>
            </motion.div>
          </>
        )}
