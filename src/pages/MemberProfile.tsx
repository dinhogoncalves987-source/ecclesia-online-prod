...
import {
  ArrowLeft, User, MapPin, Phone, Mail, Church, FileText, Users, Hash, AlertCircle,
...
import {
  ...
  createMemberOccurrence, createMemberOrdination, updateMemberOrdinationStatus, createMemberTransfer,
...
import {
  ...
  TRANSFER_STATUS_LABELS, type OccurrenceType, type OrdinationType,
  type TransferDirection, type TransferLocationType, type TransferStatus, type Visibility,
...
export default function MemberProfile() {
  ...
  const canWrite = hasCapability("members.write");
  ...
  const [occForm, setOccForm] = useState({
    occurrence_type: "outro" as OccurrenceType,
  ...
  const [ordForm, setOrdForm] = useState({
    role_or_function: "",
    ordination_type: "nomeacao" as OrdinationType,
    ordination_date: "",
  ...
  const [transferForm, setTransferForm] = useState({
    direction: "recebida" as TransferDirection,
    origin_type: "interna" as TransferLocationType,
    origin_organization_id: "",
    origin_church_name: "",
    destination_type: "interna" as TransferLocationType,
  ...
  const handleEndOrdination = async (row: MemberOrdinationRow) => {
    const { error } = await updateMemberOrdinationStatus(
      row.id,
      "encerrado",
  ...
  const handleAddTransfer = async () => {
    if (!member || !canWrite) return;
    const isReceived = transferForm.direction === "recebida";
    const counterpartyType = isReceived ? transferForm.origin_type : transferForm.destination_type;
    const counterpartyOrganizationId = isReceived
      ? transferForm.origin_organization_id
      : transferForm.destination_organization_id;
    const counterpartyChurchName = isReceived
      ? transferForm.origin_church_name
      : transferForm.destination_church_name;

    if (counterpartyType === "interna" && !counterpartyOrganizationId) {
      toast({ title: t("Selecione a unidade da transferência"), variant: "destructive" });
      return;
    }
    if (counterpartyType === "externa" && !counterpartyChurchName.trim()) {
      toast({ title: t("Informe o nome da igreja"), variant: "destructive" });
      return;
    }
    setSavingSection(true);
    const { row, error } = await createMemberTransfer({
      ...
      counterparty_organization_id: counterpartyType === "interna" ? counterpartyOrganizationId : null,
      counterparty_church_name: counterpartyType === "externa" ? counterpartyChurchName.trim() : null,
  ...
  const handleAdvanceTransferStatus = async (row: MemberTransferRow, nextStatus: TransferStatus) => {
    const { error } = await updateMemberTransferStatus(row.id, nextStatus);
    if (error) {
  ...
  if (!member) return <AdminLayout><div /></AdminLayout>;
  ...
  return (
    <AdminLayout>
      <div className="max-w-3xl mx-auto space-y-6 pb-10 px-1 sm:px-0">
      {/* Header */}
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <Badge variant={member.status === "Ativo" ? "default" : "secondary"} className="sm:ml-auto">
          {member.status}
        </Badge>
      </div>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          ...
          <InfoRow label={t("Data de nascimento")} value={formatDateOnly(member.birth_date)} />
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          ...
          <InfoRow label={t("Data de admissão")} value={formatDateOnly(member.joined_at)} />
          <InfoRow label={t("Forma de admissão")} value={member.admission_type} />
          <InfoRow label={t("Batismo nas águas")} value={formatDateOnly(member.baptized_at)} />
          <InfoRow label={t("Local do batismo")} value={member.baptism_place} />
          <InfoRow label={t("Batismo c/ Espírito Santo")} value={formatDateOnly(member.holy_spirit_baptism_date)} />
          <InfoRow label={t("Consagração")} value={formatDateOnly(member.consecration_date)} />
        ...
        <CardContent className="space-y-3 text-sm">
          ...
          {family.map((f) => (
            <div key={f.id} className="flex items-center gap-2">
              ...
              {f.birth_date && <span className="text-xs text-muted-foreground ml-auto">{formatDateOnly(f.birth_date)}</span>}
            </div>
          ))}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-2">
          ...
          {canWrite && <Button size="sm" variant="outline" onClick={() => setOccDialogOpen(true)}>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-2">
          ...
          {canWrite && <Button size="sm" variant="outline" onClick={() => setOrdDialogOpen(true)}>
            <Plus size={14} className="mr-1" />{t("Registrar")}
          </Button>}
        </CardHeader>
        <CardContent className="text-sm space-y-3">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-2">
          ...
          {canWrite && <Button size="sm" variant="outline" onClick={() => setTransferDialogOpen(true)}>
            <Plus size={14} className="mr-1" />{t("Registrar")}
          </Button>}
        </CardHeader>
        <CardContent className="text-sm space-y-3">
          {transfers.length === 0 ? (
            ...
            transfers.map((tr) => (
              <div key={tr.id} className="flex items-start gap-2 pb-2 border-b border-border/40 last:border-0 last:pb-0">
                ...
                {canWrite && tr.status !== "concluida" && tr.status !== "rejeitada" && tr.status !== "cancelada" && (
          ...
          <div className="space-y-3">
            <FormSelectLabeled
              ...
              onChange={(v) => setOccForm(f => ({
                ...f,
                occurrence_type: v as OccurrenceType,
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            <FormSelectLabeled
              ...
              onChange={(v) => setOrdForm(f => ({ ...f, ordination_type: v as OrdinationType }))}
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            <FormSelectLabeled
              ...
              onChange={(v) => setTransferForm(f => ({ ...f, direction: v as TransferDirection }))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <FormSelectLabeled
                ...
                onChange={(v) => setTransferForm(f => f.direction === "recebida"
                  ? { ...f, origin_type: v as TransferLocationType }
                  : { ...f, destination_type: v as TransferLocationType })}
              ...
              {(transferForm.direction === "recebida" ? transferForm.origin_type : transferForm.destination_type) === "interna" ? (
                <FormSelectLabeled
                  ...
                  options={subOrgs
                    .filter((s) => s.id !== (member.congregation_id || member.sector_id || member.organization_id))
