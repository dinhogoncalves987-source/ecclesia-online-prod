...
import { Library, Plus, Search, Trash2, ArrowLeft, Music2 } from "lucide-react";
...
export default function BibliotecaMusicas() {
  ...
  const [songs, setSongs] = useState<WorshipSong[]>(() => getSongs(churchId));
  ...
  const handleSave = () => {
    ...
    saveSong(churchId, { ...form, id: editingId ?? undefined });
    setSongs(getSongs(churchId));
    setShowForm(false);
    toast.success(editingId ? t("Música atualizada!") : t("Música cadastrada!"));
  };

  const handleDelete = (id: string) => {
    deleteSong(churchId, id);
    setSongs(getSongs(churchId));
      ...
      <div className="space-y-6 max-w-4xl">
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-16 bg-card rounded-2xl border border-border/50">
            ...
            <button type="button" onClick={openNew} className="mt-4 text-sm text-accent font-medium hover:underline">
          ...
          <div className="grid gap-3">
            {filtered.map((song) => (
              <div key={song.id} className="bg-card rounded-xl border border-border/50 p-4 flex items-start gap-4">
                <div className="flex-1 min-w-0 cursor-pointer" onClick={() => openEdit(song)}>
                ...
                <button type="button" onClick={() => handleDelete(song.id)} className="p-2 text-muted-foreground hover:text-destructive">
                  <Trash2 size={16} />
                </button>
              </div>
            ))}
