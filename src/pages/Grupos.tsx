    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-serif font-bold text-foreground">Pequenos Grupos</h1>
            <p className="text-sm text-muted-foreground mt-1">Gerencie os grupos de comunhão e estudo</p>
          </div>
            ...
            {groups.map((g, i) => (
              <motion.div key={g.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div className="space-y-1.5 text-xs text-muted-foreground">
                  <div className="flex items-center gap-1.5"><User size={12} /> Líder: {g.leader}</div>
                  ...
                  <div className="flex items-center gap-1.5"><UsersRound size={12} /> {g.current_members}/{g.max_members} membros</div>
                </div>
                {g.user_id === user?.id && (
                  <div className="mt-3 pt-2 border-t border-border/30 flex justify-end">
                    <button onClick={() => handleDelete(g.id)} className="text-xs text-destructive hover:bg-destructive/10 px-2 py-1 rounded transition-colors">Remover</button>
                  </div>
                )}
              </motion.div>
            ))}
