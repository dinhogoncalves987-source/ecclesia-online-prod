    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 shadow-2xl">
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="text-white font-bold text-lg">Verificação de Membro</h1>
          <p className="text-slate-400 text-xs mt-0.5">Ecclesia · Sistema de validação</p>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center gap-3 py-6">
            ...
            <p className="text-slate-400 text-sm">Consultando cadastro...</p>
          </div>
        ) : result?.ok ? (
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-center gap-2 py-2 bg-emerald-500/10 rounded-xl border border-emerald-500/20">
              ...
              <span className="text-emerald-300 font-semibold text-sm">Membro verificado</span>
            </div>

            {/* Dados */}
            <div className="space-y-3">
              <div className="border-b border-white/10 pb-3">
                <p className="text-slate-500 text-[10px] uppercase tracking-wider">Nome completo</p>
                <p className="text-white font-semibold text-base mt-0.5">{result.full_name}</p>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-slate-500 text-[10px] uppercase tracking-wider">Status</p>
                  <p className={`font-semibold text-sm mt-0.5 ${STATUS_LABEL[result.status ?? "Ativo"]?.color ?? "text-slate-300"}`}>
                    {STATUS_LABEL[result.status ?? "Ativo"]?.label ?? result.status}
                  </p>
                </div>
                <div>
                  <p className="text-slate-500 text-[10px] uppercase tracking-wider">Cargo</p>
                  <p className="text-slate-200 text-sm mt-0.5">{result.member_role ?? "Membro"}</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-slate-500 text-[10px] uppercase tracking-wider">Igreja</p>
                  <p className="text-slate-200 text-sm mt-0.5">{result.church_name}</p>
                </div>
                {result.joined_at && (
                  <div>
                    <p className="text-slate-500 text-[10px] uppercase tracking-wider">Membro desde</p>
              ...
              <div className="border-t border-white/10 pt-3">
                <p className="text-slate-500 text-[10px] uppercase tracking-wider">Matrícula</p>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-center gap-2 py-2 bg-amber-500/10 rounded-xl border border-amber-500/20">
              ...
              <span className="text-amber-300 font-semibold text-sm">Consulta indisponível</span>
            </div>
            <p className="text-slate-400 text-sm text-center leading-relaxed">
              Para verificar este membro, entre em contato com a secretaria da igreja apresentando
              o código abaixo.
            </p>
            {result?.code && (
              <div className="bg-white/5 rounded-lg p-3 text-center">
                <p className="text-slate-500 text-[10px] uppercase tracking-wider mb-1">Código de consulta</p>
        ...
        <div className="mt-6 pt-4 border-t border-white/10 text-center">
          <p className="text-slate-600 text-[10px]">
            Verificado em {now} · Documento pessoal e intransferível
          </p>
          <p className="text-slate-700 text-[10px] mt-0.5">
            Powered by Ecclesia Online
          </p>
        </div>
      </div>
    </div>
