...
CREATE TABLE IF NOT EXISTS public.member_addresses (
  ...
  address_type text NOT NULL DEFAULT 'residencial'
    CHECK (address_type IN ('residencial', 'comercial', 'correspondencia', 'anterior', 'outro')),
  zip_code text,
  street_type text,
  street text,
  number text,
...
DROP POLICY IF EXISTS "member_addresses capability select" ON public.member_addresses;
DROP POLICY IF EXISTS "member_addresses capability insert" ON public.member_addresses;
DROP POLICY IF EXISTS "member_addresses capability update" ON public.member_addresses;
DROP POLICY IF EXISTS "member_addresses capability delete" ON public.member_addresses;
...
CREATE OR REPLACE FUNCTION public._member_addresses_protect_identity()
RETURNS trigger
LANGUAGE plpgsql
...
BEGIN
  IF NEW.member_id IS DISTINCT FROM OLD.member_id
    ...
    RAISE EXCEPTION 'member_id and organization_id are immutable for member addresses';
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS member_addresses_protect_identity ON public.member_addresses;
CREATE TRIGGER member_addresses_protect_identity
BEFORE UPDATE ON public.member_addresses
FOR EACH ROW EXECUTE FUNCTION public._member_addresses_protect_identity();

REVOKE ALL ON FUNCTION public._member_addresses_protect_identity() FROM PUBLIC, anon, authenticated;
