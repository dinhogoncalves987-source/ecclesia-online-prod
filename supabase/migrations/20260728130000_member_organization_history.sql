...
BEGIN
  IF NEW.organization_id IS DISTINCT FROM OLD.organization_id THEN
    ...
    PERFORM public.register_member_history_event(
      NEW.id, 'mudanca_organizacao', 'Alteração de organização', NULL, now(),
  ...
  IF NEW.sector_id IS DISTINCT FROM OLD.sector_id THEN
    ...
    PERFORM public.register_member_history_event(
      NEW.id, 'mudanca_setor', 'Alteração de setor/distrito', NULL, now(),
      'secretaria', 'members', NEW.id, NULL, NULL, 'normal', NULL, NULL, NULL
    );
  END IF;
  ...
  IF NEW.congregation_id IS DISTINCT FROM OLD.congregation_id THEN
    ...
    PERFORM public.register_member_history_event(
      NEW.id, 'mudanca_congregacao', 'Alteração de congregação', NULL, now(),
      'secretaria', 'members', NEW.id, NULL, NULL, 'normal', NULL, NULL, NULL
    );
  END IF;
...
BEGIN
  IF NEW.status IS DISTINCT FROM OLD.status THEN
    PERFORM public.register_member_history_event(
      ...
      NULL, now(), 'secretaria', 'members', NEW.id, NULL, NULL, 'normal', NULL, NULL, NULL
    );
  END IF;
  RETURN NEW;
END;
