...
1. **Uma timeline só**: `public.member_history`, alimentada exclusivamente pela função
   ...
   A tabela é append-only: `authenticated` possui somente `SELECT`; não há `INSERT`, `UPDATE` ou
   `DELETE` direto. A função genérica também não é executável pelo navegador.
...
7. **Origem legada pronta, importação não implementada**: todas as tabelas especializadas têm
   `legacy_source`/`legacy_module`/`legacy_code` + índice único parcial de idempotência. A timeline
   usa uma chave por evento (inclui tipo e referência especializada), evitando que dois fatos
   legítimos da mesma ficha abortem uma transação. A rotina
...
8. **`register_member_history_event()` funciona também sem usuário autenticado**
   ...
   precisar de uma política "authenticated pode tudo". `EXECUTE` é concedido somente a
   `service_role`; no fluxo normal, triggers/RPCs específicas são a porta pública.
...
- `create_member_occurrence()` / `update_member_occurrence_status()` — criação e transições sem
  permitir ao cliente forjar organização/ator.
- `create_member_ordination()` / `update_member_ordination_status()` — criação e
  encerramento/revogação controlados.
- `create_member_transfer()` / `update_member_transfer_status()` — completa automaticamente o lado
  local da transferência e valida a máquina de estados.
...
- `members_track_status_change` (AFTER UPDATE OF status em `members`) — registra
  "mudança de situação".
- Backfill idempotente abre o vínculo organizacional atual dos membros que já existiam, sem
  inventar eventos retroativos.
...
- `src/lib/memberHistory.ts` — camada de serviço tipada (`loadMemberHistory`,
  `loadMemberOccurrences`/`createMemberOccurrence`,
...
|---|---|---|---|---|
| `member_history` | `members.read` (+ `members.confidential` se confidencial) | somente função interna | — | — |
| `member_occurrences` | `members.read` (+ confidential) | RPC específica | transição por RPC | — |
| `member_ordinations` | `members.read` | RPC específica | encerramento/revogação por RPC | — |
| `member_transfers` | `members.read` | RPC específica | máquina de estados por RPC | — |
...
- `register_member_history_event()` é o ponto interno do banco. O próximo módulo deve chamá-lo em
  trigger sobre sua tabela de matrícula, com `source_module = 'discipulado'`; o navegador usa uma
  RPC específica da matrícula, nunca a função genérica de auditoria.
...
`src/integrations/supabase/types.ts` (tipos das 5 tabelas + RPCs específicas e função interna de
histórico), `src/pages/MemberProfile.tsx` (4 novas seções, dentro de `AdminLayout`, com ações
condicionadas a `members.write`).
