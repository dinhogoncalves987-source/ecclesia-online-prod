...
| `legacy_source`/`legacy_module`/`legacy_code` | origem WinTechi, com índice único parcial que impede importar o mesmo registro duas vezes |
...
Esta é a **única porta de entrada** para gravar na timeline. A Secretaria já a usa (via triggers em
`member_occurrences`/`member_ordinations`/`member_transfers`/`members`). **Discipulado, Teologia e
Missões devem chamar a mesma função** (com `p_source_module` próprio) em vez de criar uma segunda
timeline. A função:

- Resolve a organização efetiva do membro (`COALESCE(congregation_id, sector_id, organization_id)`).
- Exige `members.write` quando há usuário autenticado (`auth.uid()` não nulo).
...
índice único parcial `(organization_id, legacy_source, legacy_code) WHERE legacy_code IS NOT NULL`.
...
| `public.member_history` + `register_member_history_event()` | Timeline própria do módulo |
...
4. **Registrar evento na timeline**: `register_member_history_event()` (RPC) ou
   `src/lib/memberHistory.ts#registerHistoryEvent()` (wrapper tipado no frontend).
