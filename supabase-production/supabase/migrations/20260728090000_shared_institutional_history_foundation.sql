...
SET permission_keys = array_append(permission_keys, 'members.confidential'),
    updated_at = now()
WHERE responsibility_type IN ('church_admin', 'responsible_pastor')
  AND NOT ('members.confidential' = ANY(permission_keys));
...
-- Idempotência de importação legada: o mesmo registro do WinTechi (mesma
-- origem + mesmo código) nunca gera duas linhas de histórico.
CREATE UNIQUE INDEX IF NOT EXISTS member_history_legacy_unique_idx
  ON public.member_history (organization_id, legacy_source, legacy_code)
  WHERE legacy_code IS NOT NULL AND legacy_source IS NOT NULL;

DROP TRIGGER IF EXISTS update_member_history_updated_at ON public.member_history;
CREATE TRIGGER update_member_history_updated_at
...
-- Escrita manual (nota rápida na timeline): permitida via members.write, mas
-- a MAIORIA dos eventos reais desta operação é criada automaticamente pela
-- função register_member_history_event() chamada pelos triggers das tabelas
-- especializadas (ver migrations seguintes) — não pelo INSERT direto do
-- cliente. Ainda assim a policy existe para permitir uma nota manual futura
-- sem exigir nova migration.
DROP POLICY IF EXISTS "member_history capability insert" ON public.member_history;
CREATE POLICY "member_history capability insert" ON public.member_history
FOR INSERT TO authenticated
WITH CHECK (
  EXISTS (
    SELECT 1 FROM public.members m
    WHERE m.id = member_history.member_id
      AND m.organization_id = member_history.organization_id
      AND public.has_org_access_permission(
...
-- Correção de título/descrição (typo) é permitida; NUNCA é permitido apagar
-- (mesmo padrão de organization_access_audit — trilha de auditoria não tem
-- policy de DELETE, propositalmente).
DROP POLICY IF EXISTS "member_history capability update" ON public.member_history;
CREATE POLICY "member_history capability update" ON public.member_history
...
BEGIN
  ...
  INSERT INTO public.member_history (
    ...
    p_member_id, v_org_id, p_history_type, p_title, p_description, COALESCE(p_occurred_at, now()),
