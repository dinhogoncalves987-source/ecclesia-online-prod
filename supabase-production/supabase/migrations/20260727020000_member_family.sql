...
CREATE TABLE IF NOT EXISTS public.member_family (
  ...
  -- Relacionamento
  relation text NOT NULL CHECK (relation IN ('pai', 'mae', 'esposo', 'esposa', 'filho', 'filha', 'enteado', 'enteada', 'dependente', 'responsavel', 'outro')),
...
CREATE UNIQUE INDEX IF NOT EXISTS member_family_unique_relation
  ON public.member_family (member_id, relation, lower(btrim(full_name)))
...
DROP POLICY IF EXISTS "member_family capability select" ON public.member_family;
DROP POLICY IF EXISTS "member_family capability insert" ON public.member_family;
DROP POLICY IF EXISTS "member_family capability update" ON public.member_family;
DROP POLICY IF EXISTS "member_family capability delete" ON public.member_family;
...
WITH CHECK (
  EXISTS (
    ...
    WHERE m.id = member_family.member_id
      ...
      AND (
        member_family.related_member_id IS NULL
        OR EXISTS (
          SELECT 1 FROM public.members related
          WHERE related.id = member_family.related_member_id
            AND related.organization_id = m.organization_id
        )
      )
      AND public.has_org_access_permission(
...
CREATE OR REPLACE FUNCTION public._member_family_protect_identity()
RETURNS trigger
LANGUAGE plpgsql
...
BEGIN
  IF NEW.member_id IS DISTINCT FROM OLD.member_id
    ...
    RAISE EXCEPTION 'member_id and organization_id are immutable for member family';
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS member_family_protect_identity ON public.member_family;
CREATE TRIGGER member_family_protect_identity
BEFORE UPDATE ON public.member_family
FOR EACH ROW EXECUTE FUNCTION public._member_family_protect_identity();

REVOKE ALL ON FUNCTION public._member_family_protect_identity() FROM PUBLIC, anon, authenticated;
